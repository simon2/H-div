...
int* prefix_sum_par(int* input, int length, int init);
int* prefix_sum_seq(int* input, int length, int init);
...
#pragma cilk grainsize = 128
	...
	nl += lessStart[id];
      ...
      if(nl != 0 && nl != nd){
	prefix_sum_par(lessStart,gn,0);
	prefix_sum_par(moreStart,gn,nl);
...
int* prefix_sum_par(int* input, int length, int init){
  ...
  if(length-l>1){
    prefix_sum_par(&input[l],length-l,sum);
  }else if(length-l == 1){
    input[l] = sum;
  }
  return input;
