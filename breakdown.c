...
void supermatrix_construction_cog_leafmtrx(leafmtxp *st_leafmtxp,   //the H-matrix
  ...
  printf("countsl:%ld\n",scountsl);
  //printf("breakdown: %lf %lf %lf %lf\n", scount, sprefix, scopy, sseq);
  printf("breakdown:%lf\n",sprefix);
...
#pragma cilk grainsize = 128
	...
	moreNum[id] = mn;
      ...
      spent = end - start;
      countt[workerID*1000] += spent;
      //do sth. to add all together.
      start = get_wall_time();
