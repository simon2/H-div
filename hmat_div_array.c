...
int create_ctree_ssgeom(int st_clt,double (*zgmid)[3],double param[],int ndpth,int ndscd,int nsrt,int nd,int md,int ndim,int nclst);
...
int create_ctree_ssgeom(int st_clt,   //the current node
  ...
  if(nd <= minsz){
	...
	bal[ndpth] = nd;
      ...
      int nsrt1 = nsrt;
      int nd1 = nl;
      CTlist[st_clt].offsets[0] = create_ctree_ssgeom(CTlist[st_clt].offsets[0],zgmid,param,
					       ndpth,ndscd,nsrt1,nd1,md,ndim,nclst);

      nsrt1 = nsrt + nl;
      nd1 = nd - nl;
      CTlist[st_clt].offsets[1] = create_ctree_ssgeom(CTlist[st_clt].offsets[1],&zgmid[nl],param,
					       ndpth,ndscd,nsrt1,nd1,md,ndim,nclst);
    }
  }
